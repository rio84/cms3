...
$.pagemanager = function(obj)
{
    ...
    defaultopts = {         //定义页面上各元素的选项器
        ...
        sModeEditBtn: ".btnedit", //模块编辑按钮
        sEditBtn: ".contentsign", //内容编辑按钮
...
Head = {//head对象
    ...
    Content = {//定义页面内容区对象 
        ...
        oEditBtn: $(defaultopts.sEditBtn), //编辑按钮
        fnRefresh: function()
        {
        ...
        fnAddSign: function()
        {
            ...
            Content.oSelf.each(function()
            {
                ...
                if (oTemp.children(defaultopts.sEditBtn).length == 0)
                {
            ...
            Content.oEditBtn = $(defaultopts.sEditBtn);
        },
        setCurrentContent: function(o)
        {
            ...
            $(defaultopts.sEditBtn + ":hidden").show();
    ...
    Mode = {//定义模块对象
        ...
        oTitle: {//模块标题对象
            ...
            fnEditSubTitle: function(o, type, text, href)
            {
                ///<summary>
                ///编辑子标题
                ///</summary>
                ...
                if (o)
                {
                    if (type == "link")
                    {
                        o.children("a").text(text).attr("href", href);
                    }
                    else
                    {
                        o.text(text);
        ...
        oEditBtn: $(defaultopts.sModeEditBtn),
        fnRefresh: function()
        {
            Col.fnRefresh();
    ...
    t = objtemp.offset().top - ptop - parseInt(objtemp.css("margin-top").replace("px", ""));
            ...
            {
                Mode.oSelf.css({ position: "static", left: "", right: "", top: "", bottom: "", width: "auto", height: "auto" });
        ...
        editbtn: function()
        {
            ...
            Mode.oEditBtn = $(defaultopts.sModeEditBtn).bind("click", fnBind);
        },
    ...
    CurrentOperate =
    {
        ...
        fnShow: function(o)
        {
            ...
            ///	</param>
            if (CurrentOperate.isShow)
                return;
            if (o)
                oCurrent = $(o).parent();
            getVal();
            CurrentOperate.oSelf.show();
            CurrentOperate.isShow = true;
            //test alert(oCurrent.attr("id"))
            CurrentOperate.fnShiftFunc("content");
            CurrentOperate.oCancelBtn.one("click", function()
            {//点击退出按钮
                CurrentOperate.oSelf.hide();
                CurrentOperate.isShow = false;
                CurrentOperate.fnShiftFunc("style");
            });
            CurrentOperate.fnModeChildren();
        ...
        fnModeChildren: function()
        {

            //动态生成子元素选项

            if (oCurrent[0].id.indexOf("sl_m") == 0)
            {

                var vHtml = "",
            oModeChildren = oCurrent.children("[id^='sl_m']");
                btns = CurrentOperate.oChildElements.children("fieldset").children("div").children("a");

                btns.click(function()
                {

                    switch (btns.index(this))
                    {
                        case 1:
                            oCurrent = oCurrent.children("[id$='title']");
                            break;
                        case 2:
                            oCurrent = oCurrent.children("[id$='body']");
                            break;
                        case 3:
                            oCurrent = oCurrent.children("[id$='foot']");
                            break;
                    }
        ...
        fnShiftFunc: function(type)
        {
            ...
            switch (type)
            {
                case "style": //样式管理
                    ...
                    Mode.oSelf.children("[id^='sl_m']").unbind("click", fnBind);
                    break;
                case "content": //内容管理
                    if (Content.sCurrentLayout == "free")
                    {
                    ...
                    {
                        Col.oSelf.mysort("disable");
                    }
                    var fnBind = function() { ContentManager($(this)); };


                    Mode.oSelf.children("[id^='sl_m']").bind("click", fnBind);
                    break;
            }
...
getVal = function()
{
    ...
    if (oCurrent)
    {
        ...
        {
            var arr = bg.split(" ");
            $("#txtbgHorizontal").val(arr[0])
            $("#txtbgVertical").val(arr[1])
        }
