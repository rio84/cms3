...
var CmsDemo = fn.prototype = {
        ...
        $("#asave").click(function()
        {
            ...
            for (var i = 0; i < cols.length; i++)
            {
                ...
                for (var j = 0; j < oc.length; j++)
                {
                    ...
                    var title = $.trim(oc.eq(j).children(".mode_title").children("div").text().replace('\n', '').replace('\r', ''));
                    var titledisplay = oc.eq(j).children(".mode_title").css("display");
                    laycols += "{ \"id\": \"" + id + "\", \"style\": \"" + style + "\", \"title\": \"" + title + "\",\"titledisplay\":\"" + titledisplay + "\" },";
                }
    ...
    module: {
        ...
        addTab: function(id, tabindex)//增加内容页 id为当前母版的id tabindex 代表第几位
        {
            ...
            titleObj = $.trim(CmsDemo._currentObj.children(".mode_title").children("div").text()),
            masterObj = $("#" + id),
            ...
            if (masters.length > 0)
            {
                ...
                masters.each(function(i)
                {
                    ...
                    if (temp.children("[name='sign']").length == 0)//添加标签
                        temp.append("<div name='sign' style='text-align:center;position:absolute;height:22px;width:50px;background:#000000;top:0;right:0;display:none;color:#00ffff;font-size:20px;font-weight:700;z-index:2;'>" + index + "</div>");
        ...
        recoverModule: function(obj, type)//移出模块 type 代表母版类型
        {
            ...
            if (type)
            {
                ...
                var htmObj = $("<div id='" + contentObj[0].id.replace("box_body", "mode_box") + "' style='min-width:300px;_height:120px;_width:300px;min-height:120px;position:absolute;' class='mode_box'><div class='mode_title'></div><div></div></div>");
            ...
            contentObj = contentObjs.eq(tabObjs.index(tabObj[0]));
                ...
                $(obj).remove();
                var htmObj = $("<div id='" + contentObj[0].id.replace("box_body", "mode_box") + "' style='min-width:300px;_height:120px;_width:300px;min-height:120px;position:absolute;' class='mode_box'><div class='mode_title'><div>" + tabObj.text() + "</div></div><div></div></div>");
                areaObj.append(htmObj);
        ...
        event: function()
        {
            ...
            $("#divmastermodules a").click(function()
            {
                ...
                {//母版模块的命名规则：master_lr_no1 第一个下载划线后面的字符代表类型 no后面的数字代表唯一编号
                    case "lr":
                        masterHtml = "<div id=\"master_lr_no" + i + "\" class=\"master_box mode_box\"><div class=\"master_title\"><div>master " + i + "</div></div><div class='masterleft'><div class='btnedit' onclick=\"CmsDemo.handler.showDialog(this)\"></div></div><div class='masterright'><div class='btnedit' onclick=\"CmsDemo.handler.showDialog(this)\"></div></div></div>";
                        break;
                    case "tb":
                        masterHtml = "<div id=\"master_tb_no" + i + "\" class=\"master_box mode_box\"><div class=\"master_title\"><div>master " + i + "</div></div><div class='mastertop'><div class='btnedit' onclick=\"CmsDemo.handler.showDialog(this)\"></div></div><div class='masterbottom'><div class='btnedit' onclick=\"CmsDemo.handler.showDialog(this)\"></div></div></div>";
                        break;
                    case "tab":
        ...
        implement: function()//-------------- implement
        {
            ...
            $("input[id^='txtTitleContent']").blur(function()
            {
                var v = $.trim($(this).val());
                if (v)
                    CmsDemo._currentObj.children(".mode_title").children("." + this.id.replace('txtTitleContent', 'title')).html(v);
            });
            //title
            ...
            $("input[id^='RadioTitleHide']").click(function()
            {
                ...
                {
                    CmsDemo._currentObj.children(".mode_title").children("." + this.id.replace('RadioTitleHide', 'title')).hide();
                }
            });
            $("input[id^='RadioTitleShow']").click(function()
            {
                if ($(this).attr("checked"))
                {
                    CmsDemo._currentObj.children(".mode_title").children("." + this.id.replace('RadioTitleShow', 'title')).show();
                }
            });
            //--------------background
    ...
    handler: {
        ...
        setStyle: function(obj, title, styleOpts)
        {
            if (obj.length != 1)
                return;
            CmsDemo._titleSpan = CmsDemo._currentObj.children(".mode_title").children("div");
            ...
            if (data)
            {
                ...
                $.each(data.laycols, function(i, n)
                {
                    ...
                    $.each(n.models, function(j, o)
                    {
                        ...
                        {
                            vhtm += "<div id='" + o.id + "' style='" + o.style + "' class='mode_box'><div class='master_title'><div class='title001'>" + o.title + "</div><div 'title002'>" + o.title + "</div></div><div></div></div>";

                        }
                        else
                            vhtm += "<div id='" + o.id + "' style='" + o.style + "' class='mode_box'><div class='mode_title' style='display:" + o.titledisplay + "'><div class='title001'>" + o.title + "</div><div class='title002'>" + o.title + "</div></div><div></div></div>";
                    });
...
function(imglist,pagination,time)
{    
    var i = 0;
    var list = $("#loopedSlider .container>div");
    var indexs = $("#loopedSlider .pagination a");
    if (imglist)
        list = imglist;
    if (pagination)
        indexs = pagination;
    ...
    var fnSlide = function()
    {
        list.hide();
        list.eq(i).show();
        indexs.removeClass("current").eq(i).addClass("current");
        i++;
        if (i >= list.length)
            i = 0;
    };

    setInterval(fnSlide, time);
